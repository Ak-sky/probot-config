...
// Modified code to support n level _extends
async function getConfig(context, fileName, defaultConfig, deepMergeOptions) {
  const filePath = path.posix.join(CONFIG_PATH, fileName);
  const params = context.repo({
    path: filePath,
  ...
  const config = await loadYaml(context, params);
  loadedConfig.add(getYmlFilePath(params));
  
  let extendedRepo;
  if (config == null) {
    extendedRepo = DEFAULT_BASE;
  } else if (config != null && BASE_KEY in config) {
    extendedRepo = config[BASE_KEY];
    delete config[BASE_KEY];
  }

  let extendedConfig;
  if (extendedRepo) {
  extendedConfig = await getBaseConfig(context, params, extendedRepo, deepMergeOptions);
  }

  if (config == null && extendedConfig == null && !defaultConfig) {
    return null;
  }
  const mergedConfig = deepMergeConfigs(
    [defaultConfig, extendedConfig, config],
    deepMergeOptions
  );
  return mergedConfig
}

async function getBaseConfig(context, parentParams, repo, deepMergeOptions) {
  // repo: 'repo-settings:.github/common-settings.yml'
  const params = getBaseParams(parentParams, repo);
  const ymlPath = getYmlFilePath(params);
  
  if (loadedConfig.has(ymlPath)) {
    return null;
  }

  loadedConfig.add(ymlPath);

  let config = await loadYaml(context, params);

  let extendedRepo, extendedConfig;
  if (config == null) {
    return null;
  } else if (BASE_KEY in config) {
    extendedRepo = config[BASE_KEY];
    delete config[BASE_KEY];
  }
...
function getYmlFilePath(params) {
  return path.posix.join(params.owner, params.repo, params.path);
}
